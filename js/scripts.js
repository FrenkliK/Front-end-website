...
function showMovieDetails(index) {
    ...
    if (!movieDetails || !title || !description) {
        return;
    }
    ...
    if (!galleryListenersAttached && galleryImages.length) {
...
const hoverTextLeft = document.querySelector('.hover-text-left');
const hoverTextRight = document.querySelector('.hover-text-right');

if (vipCard && hoverTextLeft && hoverTextRight) {
    vipCard.addEventListener('mouseover', function() {
        hoverTextLeft.style.display = 'block';
        hoverTextRight.style.display = 'block';
    });

    vipCard.addEventListener('mouseout', function() {
        hoverTextLeft.style.display = 'none';
        hoverTextRight.style.display = 'none';
    });
}
